<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import = "java.util.List, com.happy.volreview.model.vo.VolReview,com.happy.volreview.model.vo.VolReviewPhoto" %>
<%
	int boardNo = (int)request.getAttribute("boardNo");
	VolReview vr = (VolReview)request.getAttribute("vr");
	List<VolReviewPhoto> vrp = (List<VolReviewPhoto>)request.getAttribute("vrp");
...
<form action="<%=request.getContextPath() %>/reviewwriteend.do"
    ...
    <div class="board_wrap">
        <div class="board_title">
            <strong style="font-size: 25px;">자원봉사후기</strong>
        </div>
        <div class="board_write_wrap">
            <div class="board_write">
                ...
                <div class="title" >
                    <dl>
                        ...
                        <dd style="font-size:17px;margin-top:14px;"><input type="text" placeholder="40자 이하로 입력해주세요." name="reviewTitle" required value=<%=vr.getVntTitle() %>></dd>
                   	
                    </dl>
                </div>
                <div class="info">
                    <dl>
                        <dt style="font-size:17px">작성자</dt>
                        <dd style="font-size:17px;margin-top:14px;"><%=loginMember.getMemberId() %>
                ...
                <div class="cont">
                    <textarea rows="10" cols="100" name="summernote" id="summernote"  required><%=vr.getVntContents() %></textarea>
                </div>
                ...
                <div id="del1" style="display:inline-block;position:relative;width:150px;height:120px;margin:5px;border:1px solid #00f;z-index:1">
                 <img id = "del2" style="width:100%;height:100%;z-index:none" src="<%=request.getContextPath()%>/upload/volreview/<%=vrp.get(i).getVntPhotoRename()%>"> 
            ...
            <div class="bt_wrap" >
                ...
                <input type="button" style="font-size:17px" value="취소" onclick="location.replace('<%=request.getContextPath()%>/reviewlist.do')">
            </div>
        </div>
    </form>
</section>
...
<script>
	...
	$.each(files,(i,v)=>{
	 ...
	 if(files.length!=0){
	 	$.ajax({
		url :"<%=request.getContextPath()%>/reviewupdateend.do",
		data : form,
		type : "post",
		contentType:false,
		processData:false,
