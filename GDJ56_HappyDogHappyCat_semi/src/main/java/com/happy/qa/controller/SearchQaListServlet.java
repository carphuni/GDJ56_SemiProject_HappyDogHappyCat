...
/**
 * Servlet implementation class SearchQaListServlet
 */
@WebServlet("/searchQa.do")
public class SearchQaListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchQaListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<QaForm> list=new QaService().selectQaList(keyword,cPage, numPerpage);
		request.setAttribute("qas", list);
		
		
		String pageBar="";
		int totalData=new QaService().selectQaCount(keyword);
